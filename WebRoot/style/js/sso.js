...
function SSOController() {
	...
	//闂囷拷顪呴柊宥囩枂閻ㄥ嫬鍞寸�锟�	this.setDomain = false; // 娑撳搫鎮滄稉瀣悑鐎硅褰佹笟娑氭畱瀵拷鎯庨弬鎵閹呮畱閸欏倹鏆�
	this.feedBackUrl = '';
	this.service = 'sso';
	this.domain = 'sina.com.cn'; // sina.com.cn 閸╃喓娈戞禍褍鎼ф稉宥堫啅闂囷拷顪呯拠銉啎缂冿拷
	this.from = ''; // 鐠囬攱鐪伴惃鍕徔娴ｆ挻娼靛┃鎰剁礉濮ｆ攨ntry閺囨潙鍙挎担锟�	this.pageCharset = 'GB2312'; // 妞ょ敻娼伴惃鍕椽閻緤绱濇稉鏄忣渽閺勵垳鏁ゆ禍搴ゎ吇鐠囷拷姒涙顓绘稉绡廈2312
	this.useTicket = false; // 閻ц缍嶉幋鎰閹存牕銇戠拹銉ユ倵閺勵垰鎯佹潻鏂挎礀ticket閿涘矂绮拋銈勮礋false
	this.isCheckLoginState = false; // 閸掓繂顬婇崠鏍ㄦ閺勵垰鎯佸Λ锟界叀閻ц缍嶉悩鑸碉拷, 濡拷鐓＄亸鍡氱殶feedBackUrl ,閹碉拷浜掓妯款吇娑撳秵顥戦弻锟�	this.isUpdateCookieOnLoad = true; // 閺勵垰鎯侀崷銊ュ鏉炲�顕歫s閺傚洣娆㈤崥搴ゅ殰閸斻劍娲块弬棰佺濞嗩摡ookie閿涘矂绮拋銈勮礋閼奉亜濮╅弴瀛樻煀,婵″倹鐏夐懛顏勭暰娑斿绨￠崶鐐剁殶閸戣姤鏆熼敍宀冾嚞娑撳秷顪呴懛顏勫З閺囧瓨鏌奵ookie
	this.useIframe = true; // 姒涙顓绘担璺ㄦ暏iframe 閸嬫ost閺傜懓绱￠惃鍕瑜版洩绱濇俊鍌涚亯娑撳秷鍏橀幓鎰返feedBackUrl閿涘苯鐨㈡稉宥堝厴娴ｈ法鏁ょ拠銉︽煙瀵拷閺�綊鍘ょ純顕�拷妞ら�绗夐崙铏瑰箛閸︺劍鏋冨锝勮厬
	this.noActiveTime = 7200; // 娑撳秵妞跨捄鍐╂闂�尅绱濇妯款吇2鐏忓繑妞�
	this.autoUpdateCookieTime = 1800; // 妞ょ敻娼版稉宥呭З閺冩儼鍤滈崝銊︽纯閺傜櫛ookie閻ㄥ嫰妫块梾鏃�闂傦拷姒涙顓�0閸掑棝鎸�鐏忔垳绨�s娑撳秴浠涢弴瀛樻煀
	this.loginType = rsa; // 姒涙顓籸sa閺傜懓绱�
	this.timeoutEnable = false;
	this.crossDomain = true; // default true
	this.scriptLoginHttps = false; // 瑜版挷濞囬悽鈺琽st閺傜懓绱￠惂璇茬秿婢惰精瑙﹂崥搴礉鏉烆剚宕叉稉绨奵ript閺嶅洨顒烽弬鐟扮础閻ㄥ嫮娅ヨぐ鏇熸Ц閸氾箑宸遍崚鏈靛▏閻⑩暏ttps閻ㄥ嫭鏌熷蹇曟瑜帮拷姒涙顓绘担璺ㄦ暏wsse閺傜懓绱￠惂璇茬秿
	
	this.allowAutoFoundServerTime = false; //閺勵垰鎯侀懛顏勫З閸欐垹骞囬張宥呭閸ｃ劎顏弮鍫曟？,姒涙顓绘稉宥堝殰閸斻劌褰傞悳甯礉閼峰啿鐨棃鐐垫瑜版洟銆夐棃顤嶇瘍閸︺劋濞囬悽銊嚉js閿涘矁绻栨禍娑�闂堛垹姘ㄦ稉宥呭彠韫囧啯婀囬崝鈥虫珤閺冨爼妫挎禍锟�	this.allowAutoFoundServerTimeError = true; //閺勵垰鎯侀懛顏勫З閸欐垹骞囬張宥呭閸ｃ劎顏弮鍫曟？闁挎瑨顕�
	...
	this.failRedirect = false; //瑜版捇浜ｉ崚浼存付鐟曚浇鐑︽潪顒傛畱妞ょ敻娼伴弮璁圭礉瑜版挸澧犳い鐢告桨閺勵垰鎯侀弨顖涘瘮鐠哄疇娴嗛妴锟�
	...
	this.init = function() {
		me.setLoginType(me.loginType); // 濡拷鐓￠弰顖氭儊闂囷拷顪呮担璺ㄦ暏https
		...
		//鐠佸潡妫秄eedBackUrl濡拷鐓￠惂璇茬秿閻樿埖锟介敍瀹杄edBackUrl閸欘垵鍏樻导姘虫祮閸氭唶soLoginUrl閸欐牜濮搁幀锟�		if(me.isCheckLoginState) {
			addEventListener(window,"load",function(){me.checkLoginState();});
		}
		//鐠囨洖娴樻禒搴ㄣ�闂堫枍鑵戦崣鎴犲箛ssoServerTime閿涘奔缍旀稉铏规瑜版洜娈戦崚婵嗩瀶閺冨爼妫�
	...
	/**
	 * 濮濄倕鍤遍弫棰佽礋閻劍鍩涢崣顖濐湂閻╂牜娈戦崶鐐剁殶閸戣姤鏆熼敍瀹璷ginStatus閸欏倹鏆熸稉铏规暏閹撮娈戦惂璇茬秿閻樿埖锟�
	 * 娑撶alse閸掓瑦婀惂璇茬秿
	 * 閸氾箑鍨导姘▔缁�櫣鏁ら幋椋庢畱閻ц缍嶆穱鈩冧紖閺佹壆绮�
	 */
	this.customLoginCallBack = function(loginStatus) {
		// 鐠囥儲鏌熷▔鏇犳暠娴溠冩惂閼奉亜绻侀弶銉ョ暰娑旓拷
	};
	...
	this.login = function(username, password, savestate) {
		...
		ssoLoginTimer.start(5000, function(){
			ssoLoginTimer.clear(); // 鏉╂瑩鍣锋俊鍌涚亯娑撳秵绔婚梽銈嗘闁界噦绱濋崚娆掔箿閸掓壆娈戦惂璇茬秿娓氭繄鍔ф导姘▔缁�櫣娅ヨぐ鏇犵波閺嬶拷
	...
	this.updateCookie= function() {
		...
		var result = {};
		if (cookieExpireTime == null) {  // cookie 娑撳秴鐡ㄩ崷锟�			result = {"retcode":6102}; // not login
		} else if( cookieExpireTime < now ) { // cookie 瀹歌尙绮℃潻鍥ㄦ埂
			result = {"retcode":6203}; // cookie expired
		} else if( cookieExpireTime - cookieExpireTimeLength + updateCookieTimeHardLimit > now) { // 閸︺劎鈥栭梽鎰煑閻ㄥ嫭妞傞梻鏉戝敶娑撳秴鍘戠拋鍛婃纯閺傜櫛ookie
			result = {"retcode":6110};
		} else if(cookieExpireTime - now > me.noActiveTime) {// 缁傛槒绻冮張鐔荤箷閺冣晝娼冮崨锟�			result = {"retcode":6111};
		}
	...
	// 鐠佸墽鐤嗛棁锟筋渽楠炴寧鎸遍惃鍕勾閸э拷鍨悰锟�	this.setCrossDomainUrlList = function(urlList) {
		crossDomainUrlList = urlList;
	
	
	// 濡拷鐓￠悽銊﹀煕閸氬秶娈戞径鍥拷閻ц缍嶉弬鐟扮础
	// 楠炴湹绗栫�瑙勫复閸欙綀绻戦崶鐐诧拷閸嬫矮绨℃稉锟界昂鐏忎浇顥栭妴锟�	this.checkAltLoginNameCallback = function (data){
	...
	// 閻ц缍嶉崶鐐剁殶閸戣姤鏆�script閺嶅洨顒烽惂璇茬秿閹存潩etDomain閺傜懓绱￠惂璇茬秿閺冩湹濞囬悽銊嚉閸ョ偠鐨熼崙鑺ユ殶
	this.loginCallBack = function(result) {
		try {
		if(me.timeoutEnable && !ssoLoginTimer.isset()) return;
		ssoLoginTimer.clear();
		me.loginExtraFlag = {}; //濞撳懐鈹栨稉搴㈡拱濞嗭紕娅ヨぐ鏇犳祲閸忓磭娈戦弽鍥槕
		var loginStatus = {};
		...
		if(uid) {
			...
			// 閺堝秴濮熼崳銊ь伂閺冨爼妫跨拋鍓х枂娑撳秵顒滅涵顔炬畱鐎瑰綊鏁�
			if (loginMethodCheck && result['retcode'] == "2092" && me.allowAutoFoundServerTimeError) {
				me.setServerTime(0); // 闁插秶鐤嗛張宥呭閸ｃ劍妞傞梻瀵告畱閸婄》绱濇笟澶哥艾闁插秵鏌婇懢宄板絿
				me.loginExtraFlag = objMerge(me.loginExtraFlag,{"wsseretry":"servertime_error"}); // 濞ｈ濮為柌宥堢槸閺嶅洩鐦戦敍灞肩┒娴滃簼绨＄憴锝嗗剰閸愶拷
				loginMethodCheck(); // 闁插秵鏌婇惂璇茬秿娑擄拷顐奸敍宀冪箹濞嗏剝鐥呴張澶夌啊閻ц缍嶇搾鍛閻ㄥ嫭甯堕崚鏈电啊閿涘苯鐤勯悳鎷屾崳閺夈儵澶悜锕侊拷娑撴梹鐥呴張澶婄箑鐟曪拷
				loginMethodCheck = null; //闁灝鍘ゅ璇叉儕閻滐拷
				return false;
			}
			loginStatus['result'] = false;
			loginStatus['errno'] = result['retcode'];
			if (loginStatus['errno'] == '4069') { //鐏忔繆鐦捄瀹犳祮鏉╂稖顢戞０婵嗩檱鐠併倛鐦�
	...
	this.updateCookieCallBack = function (result)  {
		...
		} else { // 韫囩晫鏆愰弴瀛樻煀婢惰精瑙﹂惃鍕剰閸愮绱濋崶鐘辫礋閺囧瓨鏌婃径杈Е閸氬海娈戦幙宥勭稊娑撳秵妲歌箛鍛淬�閻拷
			me.customUpdateCookieCallBack(result);
		}
	};
	this.feedBackUrlCallBack = function (result) {
		// 鐎甸�绨紒娆庣啊feedBackUrl娴ｅ棙妲搁崣鍫濆繁閸掔et閺傜懓绱￠惂璇茬秿閻ㄥ嫭妞傞崐娆欑礉鐠佲剝妞傞崳銊ユ躬loginCallBack閸ョ偠鐨熼弮璺哄嚒缂佸繑绔婚梽銈勭啊
		if(loginMethod == "post" && me.timeoutEnable && !ssoLoginTimer.isset()) return;
		ssoLoginTimer.clear();
		if (result.errno == "2092") {
			me.setServerTime(0);	  // 闁插秶鐤嗛張宥呭閸ｃ劍妞傞梻瀵告畱閸婄》绱濇笟澶哥艾闁插秵鏌婇懢宄板絿
		}
		if (result.errno == '4069') { // 闂囷拷顪呯捄瀹犳祮鏉╂稖顢戞０婵嗩檱鐠併倛鐦�
			var reason = result.reason.split('|');
		...
		removeNode(me.loginFrameName);// 閸掔娀娅庨悽銊ョ暚閻ㄥ埇frame,娴ｅ棙妲告稉锟界暰娑撳秷顪呴崷銊ユ礀鐠嬪啫澧犻崚鐘绘珟閿涘瓗irefox閸欘垵鍏橀張澶愭６妫版娈�
	};
	...
	this.crossDomainCallBack = function(result) {
		// 婵″倹鐏夌�妯烘躬閼惧嘲褰囬獮鎸庢尡閸︽澘娼冮崚妤勩�閻ㄥ嫯顕Ч鍌滄畱script閺嶅洨顒烽敍灞藉灟鏉╂瑩鍣峰鑼病閻劏绻冮敍灞藉灩闂勩倓绠�
		...
		if(arrURL.length == 0) { // 閺堬拷鍨垫稉宥夋付鐟曚線锟介惌銉ゆ崲娴ｆ洖鐓�
			clearTimeout(crossDomainTimer);
			me.crossDomainResult();
			return true;
		}
		for(var i = 0; i < arrURL.length; i++) {
			...
			url = makeURL(url, request);
			if(isSafari()){
				//safari 闂囷拷顪呴悽銊ㄧ箹娑擃亝鏌熷▔锟�/ http://wiki.intra.sina.com.cn/pages/viewpage.action?pageId=6297546
	...
	this.crossDomainResult = function () {
		// 濞撳懐鈹栭獮鎸庢尡閸掓銆冮敍宀勪缉閸忓秳绗呭▎陇顕ら悽锟�		crossDomainUrlList = null;
	...
	this.crossDomainAction = function (action,callback) {
		...
		if (crossDomainUrlList) { // 瀹歌尙绮＄�妯烘躬娴滃棝娓剁憰浣哥畭閹绢厾娈戦崷鏉挎絻閸掓銆�
			me.crossDomainCallBack(crossDomainUrlList);
			return false;
		}
		// 鏉╂ɑ鐥呴張澶愭付鐟曚礁绠嶉幘顓犳畱閸︽澘娼冮崚妤勩�閿涘矁鍤滃閬嶏拷鏉╁樈rossdomain.php 閺夈儴骞忛崣锟�		var domain = me.domain;
		if(action == "update") {
			action = "login";
			domain = "sina.com.cn";
		}
	...
	this.setLoginType = function(loginType) {
		// 濡拷鐓¤ぐ鎾冲妞ょ敻娼伴弰顖氭儊https閿涘苯顪嗛弸婊勬Ц閿涘苯鍨鍝勫煑娴ｈ法鏁ttps閻ц缍嶉敍灞芥儊閸掓瑦绁荤憴鍫濇珤閺堝褰佺粈锟�		var https = location.protocol == "https:"?me.https:0;
	...
	this.setServerTime = function(servertime) {
		...
		// 闁插秶鐤嗛張宥呭閸ｃ劍妞傞梻锟�		if (servertime == 0) {
			ssoServerTimeTimer.clear();
			me.servertime = servertime;
			return true;
		}
		// 闁灝鍘ょ拋鍓х枂娑撳秹娼拫杈╂畱閺冨爼妫�
	...
	//濮濄倕鍤遍弫鎵暏娴滃氦绻樼悰灞肩濞嗩摨frame鐠囬攱鐪�
	...
	var makeRequest = function(username, password, savestate) {
		...
		request["su"] = sinaSSOEncoder.base64.encode(urlencode(username)); // su閺勭椃sername閻ㄥ嫬鍩嗛崥锟�		if (me.service) request["service"] = me.service;
		if((me.loginType & rsa) && me.servertime && sinaSSOEncoder && sinaSSOEncoder.RSAKey){
			...
			password = sinaSSOEncoder.hex_sha1("" + sinaSSOEncoder.hex_sha1(sinaSSOEncoder.hex_sha1(password)) + me.servertime + me.nonce); // 缁屽搫鐡х粭锔胯娑撹桨绨￠柆鍨帳閸氬嫰銆嶉崗銊╁劥閺勵垱鏆熺�妤佹闁姵鍨氶弫鏉跨摟閻╃濮為懓灞肩瑝閺勵垰鐡х粭锔胯闁剧偓甯撮惃鍕剰閸愶拷
		}
		request["sp"] = password; // sp閺勭棷assword閻ㄥ嫬鍩嗛崥锟�		return request;
	};
	// login by script
	...
	var createForm = function (formName,display) {
		...
		// 濞ｈ濮炴稉锟介嚋濞ｈ濮為崗鍐閻ㄥ嫭鏌熷▔锟�		form.addInput = function(name,value,type) {
			if (type == null) type = 'text';
			var _name = this.getElementsByTagName('input')[name];
			if (_name ) { // 闁灝鍘ら柌宥咁樉濞ｈ濮�
	...
	//閸掋倖鏌囬弰顖氭儊娑擄拷safari 濞村繗顬囬崳锟�	var isSafari = function(){
